...
import SizeSystemsSection from '../../../categories/presentation/components/SizeSystemsSection';
import type { CategoryModel } from '../../../categories/domain/category.model';

export default function CategoriesAdminPage() {
  const { categories, isLoading, fetchCategories, deleteCategory, fetchSizeSystems } = useCategoriesStore();
  const [modalOpen, setModalOpen] = useState(false);
  const [editingCategory, setEditingCategory] = useState<CategoryModel | null>(null);

  useEffect(() => { fetchCategories(); fetchSizeSystems(); }, [fetchCategories, fetchSizeSystems]);
    ...
    <div id="categories-admin-page">
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-default-400 mt-1">Gestiona las categorías de productos y los sistemas de tallas</p>
        </div>
      </div>

      <Tabs aria-label="Categorías y Medidas" color="primary" variant="underlined" classNames={{ tabList: "gap-6" }}>
        {/* Tab: Categorías */}
        <Tab key="categories" title={
          <div className="flex items-center gap-2">
            ...
            Categorías
            <span className="text-xs text-default-400">({categories.length})</span>
          </div>
        }>
          <div className="pt-4">
        ...
        {/* Tab: Sistemas de Tallas */}
        <Tab key="size-systems" title={
          <div className="flex items-center gap-2">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
