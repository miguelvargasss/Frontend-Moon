    ...
    <div id="products-admin-page">
      ...
      {isLoading && products.length === 0 ? (
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] gap-4">
          {filtered.map((prod) => {
            // Thumbnail: single → product images, multiple → first style image
            const thumbnail = prod.productType === 'single'
              ? prod.images[0]?.url
              : prod.styles[0]?.images[0]?.url;
            const imageCount = prod.productType === 'single'
              ? prod.images.length
              : prod.styles.reduce((sum, s) => sum + s.images.length, 0);
            // Price display
            const priceDisplay = prod.productType === 'single'
              ? `S/ ${prod.price ?? 0}`
              : prod.styles.length > 0
                ? `Desde S/ ${Math.min(...prod.styles.flatMap(s => s.variants.map(v => v.price)))}`
                : 'Sin precio';
            ...
            return (
              <Card key={prod.id} shadow="none" className="border border-default-200 hover:border-primary/30 transition-colors">
                <div className="relative aspect-[4/3] overflow-hidden bg-default-100">
                  {thumbnail ? (
                    <img src={thumbnail} alt={prod.name} className="w-full h-full object-cover" />
                  ) : (
                  ...
                  <div className="absolute top-2 left-2 flex gap-1">
                    <Chip size="sm" color={prod.totalStock > 0 ? 'success' : 'danger'} variant="flat">{prod.totalStock > 0 ? 'Disponible' : 'Agotado'}</Chip>
                    <Chip size="sm" variant="flat" className="bg-black/50 backdrop-blur-sm text-white">
                      {prod.productType === 'single' ? 'Único' : 'Múltiple'}
                    </Chip>
                  </div>
                  {prod.categoryId && (
                  ...
                  {imageCount > 1 && (
                    <span className="absolute bottom-2 right-2 text-xs bg-black/60 backdrop-blur-sm text-foreground px-2 py-0.5 rounded-md">+{imageCount - 1} fotos</span>
                  )}
                </div>
                <CardBody className="gap-2">
                  <h3 className="text-sm font-semibold text-foreground line-clamp-1">{prod.name}</h3>
                  {prod.productType === 'single' && prod.variants.length > 0 && (
                    <p className="text-xs text-default-400 line-clamp-1">
                      {[...new Set(prod.variants.map((v) => v.sizeLabel).filter(Boolean))].join(', ')}
                    </p>
                  )}
                  {prod.productType === 'multiple' && prod.styles.length > 0 && (
                    <p className="text-xs text-default-400 line-clamp-1">
                      {prod.styles.map(s => s.name).join(' · ')}
                    </p>
                  )}
